...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "ailand_pos");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "ailand_pos");
  }

  // 设置默认窗口大小为 1920x1080
  gtk_window_set_default_size(window, 1920, 1080);
  
  // 设置最小窗口大小为 960x540 (保持16:9比例)
  GdkGeometry geometry;
  geometry.min_width = 960;
  geometry.min_height = 540;
  geometry.min_aspect = 16.0 / 9.0;  // 最小宽高比
  geometry.max_aspect = 16.0 / 9.0;  // 最大宽高比
